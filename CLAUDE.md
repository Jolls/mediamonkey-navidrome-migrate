...
## Changelog
Format follows [Keep a Changelog](https://keepachangelog.com/en/1.1.0/). Update CHANGELOG.md once per PR/merge (not per commit); all branch changes under one version entry, grouped under `### Added`/`### Changed`/`### Fixed`/`### Removed`/`### Security`/`### Deprecated` subheadings (only include the subheadings you need). Always bump the version — never leave an entry under `[Unreleased]`. Format:
```
## [0.1.X] - YYYY-MM-DD
### Added
- <one-line summary>
