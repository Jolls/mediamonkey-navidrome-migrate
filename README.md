...
| `Played` table (per-play history log) | *(not migratable)* | Navidrome's `annotation.play_date` only stores the single most recent play time, not full history, so there's nowhere to migrate this data to. |
